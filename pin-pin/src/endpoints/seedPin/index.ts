...
import { imageHero1 } from '../seed/image-hero-1'
...
export const seed = async ({
  ...
  payload.logger.info('Seeding Pin Pin database')
  const pin1Doc = await payload.create({
    collection: 'pins',
    data: {
      title: 'Pin1',
      user: demoAuthor.id,
  ...
  const pin2Doc = await payload.create({
    collection: 'pins',
    data: {
      title: 'Pin2',
      user: demoAuthor.id,
  ...
  const pin3Doc = await payload.create({
    collection: 'pins',
    data: {
      title: 'Pin3',
      user: demoAuthor.id,
  ...
  payload.logger.info('Seeding Events and Alerts database')

  await payload.create({
    collection: 'marketplaceEvents',
    data: {
      pin: pin1Doc.id,
      alertMedium: {
        email: true,
